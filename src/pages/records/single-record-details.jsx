...
const SingleRecordDetails = () => {
  ...
  const processTreatmentPlan = async () => {
    ...
    const response = await result.response;
    const text = response.text(); //only this causes error as this gives ```json ___response__ ```
    console.log("Raw Response:", text); //printing (in console) raw response which includes ```json ___response__ ```

    const cleanedText = text.replace(/```json|```/g, "").trim(); //this remove the ```json ___response__ ``` and provides in propoer Json foramt

    console.log("Cleaned response:", cleanedText); //printing (in console) cleaned response (json format)

    const parsedResponse = JSON.parse(cleanedText);

    console.log("parsedResponse", parsedResponse); //printing parsed cleaned text)
    const updatedRecord = await updateRecord({
      documentID: state.id,
